...
const LoanAdministratorDisbursement: React.FC = () => {
  ...
  const [pendingRecords, setPendingRecords] = useState<DetailedDisbursementRecord[]>([
    {
      ...
      sanctionAmt: "₹12,50,000",
      postSanctionDate: new Date("2024-03-13"),
      appointmentFixed: false,
      appointmentDate: new Date("2024-03-13"),
      appointmentTime: "11:00 AM",
      documentation: false,
      documentationDate: new Date("2024-03-13"),
      disbursementDone: "₹12,00,000",
      utr: "#100000"
    },
    {
      leadId: "#13124",
    ...
    },
    {
      ...
      income: true,
      lnt: true,
      requestedAmount: "₹1,50,00,000",
      sanctionAmt: "₹15,25,000",
  ...
  // Define filter columns for pending disbursements
  const pendingFilterColumns = [
    { key: "leadName", label: "Name", type: "text" as const },
    { key: "leadId", label: "Lead ID", type: "text" as const },
    { key: "bankName", label: "Bank Name", type: "select" as const, options: ["SBI", "HDFC", "ICICI"] }
  ];

  // Define filter columns for completed disbursements
  const completedFilterColumns = [
    { key: "leadName", label: "Name", type: "text" as const },
    { key: "leadId", label: "Lead ID", type: "text" as const },
    { key: "bankName", label: "Bank Name", type: "select" as const, options: ["HDFC", "SBI", "ICICI"] }
  ];
  ...
  const handleLeadNameClick = (leadName: string): void => {
    navigate(`/loan-administrator-document?leadName=${encodeURIComponent(leadName)}`);
  };

  // Filter the pending data based on search term and column filters
  const filteredPendingRecords = useMemo(() => {
    return pendingRecords.filter(item => {
      ...
      const matchesFilters = Object.entries(filters).every(([key, value]) => {
        ...
        const itemValue = item[key as keyof DetailedDisbursementRecord]?.toString().toLowerCase() || "";
        return itemValue.includes(value.toLowerCase());
      });

      return matchesSearch && matchesFilters;
    });
  }, [pendingRecords, searchTerm, filters]);

  // Filter the completed data based on search term and column filters
  const filteredCompletedRecords = useMemo(() => {
    return completedRecords.filter(item => {
      // Search term filter
      const matchesSearch = searchTerm === "" || 
        item.leadName.toLowerCase().includes(searchTerm.toLowerCase()) ||
        item.leadId.toLowerCase().includes(searchTerm.toLowerCase());
      ...
      const matchesFilters = Object.entries(filters).every(([key, value]) => {
        if (!value) return true;
        const itemValue = item[key as keyof CompletedDisbursementRecord]?.toString().toLowerCase() || "";
        return itemValue.includes(value.toLowerCase());
      });
            ...
            <tr key={index} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
              ...
              <td className="px-3 py-4 whitespace-nowrap">
                <Select
                  ...
                  onValueChange={(value) => {
                    const originalIndex = pendingRecords.findIndex(record => record.leadId === item.leadId);
                    handleInputChange(originalIndex, 'disbursementType', value);
                  }}
                >
                          ...
                          <label key={doc} className="flex items-center space-x-2 cursor-pointer">
                            <input 
                              ...
                              onChange={(e) => {
                                ...
                                handlePendingDocChange(originalIndex, newDocs);
                              }}
                              className="rounded"
                            />
              ...
              <td className="px-3 py-4 whitespace-nowrap">
                <Switch 
                  ...
                  onCheckedChange={(checked) => {
                    const originalIndex = pendingRecords.findIndex(record => record.leadId === item.leadId);
                    handleToggleChange(originalIndex, 'hardCopy', checked);
                  }}
              ...
              <td className="px-3 py-4 whitespace-nowrap">
                <Switch 
                  ...
                  onCheckedChange={(checked) => {
                    const originalIndex = pendingRecords.findIndex(record => record.leadId === item.leadId);
                    handleToggleChange(originalIndex, 'verificationInitiate', checked);
                  }}
                  className="data-[state=checked]:bg-green-500"
                />
              </td>
              ...
              <td className="px-3 py-4 whitespace-nowrap">
                <Switch 
                  ...
                  onCheckedChange={(checked) => {
                    const originalIndex = pendingRecords.findIndex(record => record.leadId === item.leadId);
                    handleToggleChange(originalIndex, 'scan', checked);
                  }}
                  className="data-[state=checked]:bg-green-500"
                />
              </td>
              <td className="px-3 py-4 whitespace-nowrap">
                <Switch 
                  ...
                  onCheckedChange={(checked) => {
                    const originalIndex = pendingRecords.findIndex(record => record.leadId === item.leadId);
                    handleToggleChange(originalIndex, 'raas', checked);
                  }}
                  className="data-[state=checked]:bg-green-500"
                />
              </td>
              <td className="px-3 py-4 whitespace-nowrap">
                <Switch 
                  ...
                  onCheckedChange={(checked) => {
                    const originalIndex = pendingRecords.findIndex(record => record.leadId === item.leadId);
                    handleToggleChange(originalIndex, 'rlms', checked);
                  }}
                  className="data-[state=checked]:bg-green-500"
                />
              </td>
              <td className="px-3 py-4 whitespace-nowrap">
                <Switch 
                  ...
                  onCheckedChange={(checked) => {
                    const originalIndex = pendingRecords.findIndex(record => record.leadId === item.leadId);
                    handleToggleChange(originalIndex, 'cod', checked);
                  }}
                  className="data-[state=checked]:bg-green-500"
                />
              </td>
              <td className="px-3 py-4 whitespace-nowrap">
                <Switch 
                  ...
                  onCheckedChange={(checked) => {
                    const originalIndex = pendingRecords.findIndex(record => record.leadId === item.leadId);
                    handleToggleChange(originalIndex, 'poAssigned', checked);
                  }}
                  className="data-[state=checked]:bg-green-500"
                />
              </td>
              <td className="px-3 py-4 whitespace-nowrap">
                <Switch 
                  ...
                  onCheckedChange={(checked) => {
                    const originalIndex = pendingRecords.findIndex(record => record.leadId === item.leadId);
                    handleToggleChange(originalIndex, 'income', checked);
                  }}
                  className="data-[state=checked]:bg-green-500"
                />
              </td>
              <td className="px-3 py-4 whitespace-nowrap">
                <Switch 
                  ...
                  onCheckedChange={(checked) => {
                    const originalIndex = pendingRecords.findIndex(record => record.leadId === item.leadId);
                    handleToggleChange(originalIndex, 'lnt', checked);
                  }}
                  className="data-[state=checked]:bg-green-500"
                />
              </td>
              ...
              <td className="px-3 py-4 whitespace-nowrap">
                <Switch 
                  ...
                  onCheckedChange={(checked) => {
                    const originalIndex = pendingRecords.findIndex(record => record.leadId === item.leadId);
                    handleToggleChange(originalIndex, 'appointmentFixed', checked);
                  }}
                  className="data-[state=checked]:bg-green-500"
                />
              </td>
              ...
              <td className="px-3 py-4 whitespace-nowrap">
                <Select
                  ...
                  onValueChange={(value) => {
                    const originalIndex = pendingRecords.findIndex(record => record.leadId === item.leadId);
                    handleInputChange(originalIndex, 'appointmentTime', value);
                  }}
                >
                  <SelectTrigger className="w-24 text-xs">
              ...
              <td className="px-3 py-4 whitespace-nowrap">
                <Switch 
                  ...
                  onCheckedChange={(checked) => {
                    const originalIndex = pendingRecords.findIndex(record => record.leadId === item.leadId);
                    handleToggleChange(originalIndex, 'documentation', checked);
                  }}
                  className="data-[state=checked]:bg-green-500"
                />
              </td>
